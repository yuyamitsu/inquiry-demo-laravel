...
ローカル開発環境では、扱いやすい SQLite を使用しています。
公開環境では、スターレンタルサーバ上の MySQL / MariaDB を使用しています。
...
DB構造は Migration、サンプルデータは Seeder で再現できるようにしています。

※ `database/database.sqlite` は環境ごとに作成するDBファイルのため、Git管理対象外としています。
※ `.env` も環境ごとの設定ファイルのため、Git管理対象外としています。
...
また、Draw.ioで読み込めるER図も作成し、現在のDB構成を視覚的に確認できるようにしています。
...
優先度と対応期限は一覧画面・詳細画面の両方でバッジ表示し、対応すべき問い合わせを見つけやすくしています。
...
* Seederによるサンプルデータ投入
* SQLiteによるローカルDB管理
* MySQL / MariaDBによる公開環境DB管理
* DB Browser for SQLiteでのDB確認
* phpMyAdminでの公開環境DB確認
...
担当者検索では、特定の担当者だけでなく、担当者がまだ設定されていない問い合わせも検索できます。
...
一覧画面では、詳細ボタンを廃止し、件名をクリックすると詳細画面へ遷移する構成にしています。

これにより、一覧画面の列数を減らし、管理項目を確認しやすくしています。

また、優先度と対応期限はバッジ表示にしています。

| 表示                 | 内容                         |
| ------------------ | -------------------------- |
| 優先度：低 / 中 / 高 / 緊急 | 優先度に応じたバッジ表示               |
| 期限切れ               | 対応期限が過ぎていて、クローズされていない問い合わせ |
| 今日まで               | 対応期限が今日の問い合わせ              |
| 未設定                | 優先度または対応期限が未設定の状態          |

詳細画面でも同じバッジ表示を行い、一覧と詳細で見た目を統一しています。

長い問い合わせ内容や、スペースのない長い文字列が入力された場合でも、カード外へはみ出さないように折り返し表示を調整しています。
...
`InquirySeeder` では、以下の動作確認ができるようにサンプルデータを用意しています。
...
ローカル開発環境では SQLite を使用しています。

SQLiteは `database/database.sqlite` という1つのファイルをDBとして扱えるため、環境構築が簡単です。

一方で、公開環境ではスターレンタルサーバ上の MySQL / MariaDB を使用しています。

| 環境     | DB              | 目的                         |
| ------ | --------------- | -------------------------- |
| ローカル開発 | SQLite          | 手軽に開発・動作確認するため             |
| 公開環境   | MySQL / MariaDB | 公開サーバー上で本番に近い構成として動作確認するため |

SQLiteは小規模Demoやローカル開発には便利ですが、複数人利用や公開環境ではMySQL / MariaDBの方が運用しやすいため、公開環境はMySQL / MariaDBへ移行しています。
...
公開環境では、スターレンタルサーバの MySQL / MariaDB を使用しています。
...
※ DB名、ユーザー名、パスワードはサーバーパネルで作成・確認したものを使用します。

---

### Migration・Seeder実行

初回構築時、または公開側DBを作り直す場合は以下を実行します。

```bash
php artisan migrate:fresh --seed
```

既存データを残してMigrationだけ反映する場合は以下を実行します。
...
これにより、ブラウザからのアクセスは `public_html/index.php` を入口にしつつ、Laravel本体は公開ディレクトリ外に置く構成にしています。
...
DBを初期化してSeederを入れ直す場合は以下を実行します。

```bash
php artisan migrate:fresh --seed
```

ただし、公開環境で `migrate:fresh --seed` を実行すると既存データが削除されるため、実行前にデータを消してよいか確認します。
...
本番運用を目的としたものではなく、設計・実装の流れ、Laravelの基本構成、DB保存の仕組み、認証、権限制御、コメントスレッド、担当者・優先度・対応期限管理、MySQL公開環境、公開手順を理解するための学習用・提案用Demoとして作成しています。
